#Day 1

Cybersecurity (also known as security) is the practice of ensuring confidentiality, integrity, and availability of information by protecting networks, devices, people, and data from unauthorized access or criminal exploitation.


| Term                   | Simple Meaning                                             |
| ---------------------- | ---------------------------------------------------------- |
| **Compliance**         | Following rules and laws                                   |
| **Security Framework** | A guide for protecting systems and data                    |
| **Security Controls**  | Tools and methods that reduce security risks               |
| **Security Posture**   | How strong an organization's security is                   |
| **Threat Actor**       | A person or group that performs cyber attacks              |
| **Internal Threat**    | A security risk from someone inside the organization       |
| **Network Security**   | Protecting a computer network from attacks                 |
| **Cloud Security**     | Protecting data and applications stored in the cloud       |
| **Programming**        | Writing code to make computers perform tasks automatically |
